...
include 'db.php'; 
// Set the `service_id` . Adjust `service_id` as needed.
$music_entertainment_service_id = 3;
?>
<!DOCTYPE html>
<html>
<head>
    <title>QuickQuote - View Music & Entertainment Services</title>
...
<body>
    ...
    <main class="container mx-auto py-10">
        <h2 class="text-3xl font-bold mb-6 text-center">Entertainment Services</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <?php
            // Query to fetch packages where `service_id` matches music and entertainment's ID
            $query = "SELECT * FROM packages WHERE service_id = ?";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("i", $music_entertainment_service_id); // Bind `service_id` value
            $stmt->execute();
