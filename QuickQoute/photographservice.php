...
<html>
<head>
    <title>QuickQuote - Photography Services</title>
...
<body>
   ...
   <div class="container mx-auto flex justify-end">
    ...
    <main class="container mx-auto py-10">
        <h2 class="text-3xl font-bold mb-6 text-center">Photography Services</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <?php
            // Include your database connection
            include 'db.php';

            // Set the `service_id` for photography. Adjust `service_id` as needed.
            $photography_service_id = 2; // Replace with the actual ID for photography in your `services` table.

            // Query to fetch packages where `service_id` matches photography's ID
            $query = "SELECT * FROM packages WHERE service_id = ?";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("i", $photography_service_id); // Bind `service_id` value
            $stmt->execute();
