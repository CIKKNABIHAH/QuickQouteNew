...
$makeup_service_id = 7; // Replace with the actual ID for catering in your `services` table.
?>

<html>
<head>
    <title>QuickQuote - Makeup Services</title>
...
<body>
   ...
   <div class="container mx-auto flex justify-end">
    ...
    <main class="container mx-auto py-10">
        <h2 class="text-3xl font-bold mb-6 text-center">Makeup Services</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <?php
            // Include your database connection
            include 'db.php';

            // Set the `service_id` for Makeup. Adjust `service_id` as needed.
            $makeup_service_id = 7; // Replace with the actual ID for Makeup in your `services` table.

            // Query to fetch packages where `service_id` matches Makeup's ID
            $query = "SELECT * FROM packages WHERE service_id = ?";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("i", $makeup_service_id); // Bind `service_id` value
            $stmt->execute();
