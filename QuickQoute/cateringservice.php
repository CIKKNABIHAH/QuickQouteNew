...
$catering_service_id = 1; // Replace with the actual ID for catering in your `services` table.
?>
<!DOCTYPE html>
<html>
<head>
    <title>QuickQuote - Catering Services</title>
    ...
    <style>
        ...
        .service-card img { width: 100%; height: auto; border-radius: 0.5rem; margin-bottom: 1rem; }
    </style>
</head>
<body>
    ...
    <main class="container mx-auto py-10">
        <h2 class="text-3xl font-bold mb-6 text-center">Catering Services</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <?php
            // Query to fetch packages where `service_id` matches catering's ID
            $query = "SELECT * FROM packages WHERE service_id = ?";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("i", $catering_service_id); // Bind `service_id` value
            $stmt->execute();
